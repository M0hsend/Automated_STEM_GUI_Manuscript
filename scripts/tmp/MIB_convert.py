            ...
            try:
                ...
                if submit_ptyrex_job:
                    ...
                    if os.path.exists(script_folder) != True:
                        os.mkdir(script_folder)
                    bash_list = _create_ptyrex_bash_submit([current_file], script_folder, 'Blackwell', '48:00:00', verbose=False)
...
def gen_config(template_path, dest_path, config_name, meta_file_path, factor=1.7, overwrite = False, verbose=False):
    ...
    with h5py.File(meta_file_path, 'r') as microscope_meta:
        try:
            if verbose:
                print(f'checking whether data is')
        except:
            pass
    ...
    for num, x in enumerate(json_files):
        ...
        if True:
            with open(bash_ptyrex_path[num], 'w') as f:
                ...
                f.write('#SBATCH --nodes 1\n')
                f.write(f'#SBATCH --tasks-per-node={taskpN}\n')
                f.write('#SBATCH --cpus-per-task 1\n')
                f.write(f'#SBATCH --gpus-per-node={gpus}\n')
                ...
                f.write(f"srun --ntasks-per-node={taskpN} --cpus-per-task=1 ptyrex_recon -c '{json_files[num]}'\n")
...
def delete_ptycho_flagging_files(basedir,year,session,subfolder,start=0,stop=-1,verbose=False):
    ...
    for num,file in enumerate(sorted(list(glob.glob('*/*/*/*autoptycho_is_done.txt')))[start:stop]):
...
def ptyrex_json_helper(json_name,val):
    ...
    try:
        if json_name == 'camera length':
            ...
            keys = ['process','common','scan','dR']
            val = [val,val]
        elif json_name == 'N':
            keys = ['process','common','scan','N']
            val = [val,val]
        elif json_name == 'bit_depth':
            keys = ['experiment','data','bit_depth']
            val = int(val)
...
def edit_ptyrex_json_parameter(json_path,key_name, new_val,verbose=False):
    ...
    with open(json_path, 'r') as f:
        ...
        print(f'Using this key list: {keys}')
        ...
        if keys[-1] == 'slices' or keys[-1] == 'sx' or keys[-1] == 'iterations':
            new_val = int(new_val)
...
def edit_all_ptycho_json_paramter(year,session,subfolder,json_name,key_name, new_val, verbose=False):
    ...
    for num,file in enumerate(sorted(list(glob.glob('*/*/*/*' + json_name + '.json')))):
        ...
        edit_ptyrex_json_parameter(os.path.join(dest_path,file),key_name, new_val, verbose=verbose)
# Ptycho Funtion End----------------------------------------------------------------------
# Function Zone End-----------------------------------------------------------------------
